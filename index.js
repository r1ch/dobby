...
const spDataHandler = connection => data => {
	...
	if(json.State && json.State.playstate){
		...
		let ping = `{"State": {"ping": {"clientRtt": 0, "clientLatencyCalculation": ${json.State.ping.clientLatencyCalculation}, "latencyCalculation": ${Date.now()/1000}}, "playstate": {"paused": false, "position": ${json.State.playstate.position}}}}\r\n`;
		console.log(ping)
